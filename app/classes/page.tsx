...
export default function ClassesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-ink">Classes</h1>
        <p className="mt-1 text-sm text-ink-soft">Les classes dont vous assurez le suivi cette année.</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                <span className="font-medium text-ink">{counts.total} élèves</span>
                {counts.attention > 0 && <span className="text-attention">{counts.attention} en attention</span>}
                {counts.aSurveiller > 0 && <span className="text-watch">{counts.aSurveiller} à surveiller</span>}
              </div>
            </Link>
          );
        })}
