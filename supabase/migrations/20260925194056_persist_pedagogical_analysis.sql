  ...
  loop
    ...
    if not exists (
      ...
      where q.id = v_question_id
        ...
        and position(sr.response_text in (v_error->>'evidenceExcerpt')) = 0
    ) then
      -- Check again using the intended direction for substring matching.
      if not exists (
        select 1
        from public.assessment_questions q
        join public.student_responses sr
    ...
    if not exists (select 1 from public.curriculum_nodes n where n.id = v_node_id and n.active) then
